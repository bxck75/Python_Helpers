...
class Core:
    ...
    def __init__(self):
        '''int objects'''
        self.root_dirname, self.root_filename = os.path.split(os.path.abspath(self.__file__))
...
#         self.C = BigHelp
    ...
    def flickr_scrape(self,search_list,img_dir,qty):
        ...
        img_list = self.H.Me(['globx',str(M.root_dirname)+'/images','*.jpg'])
        print(len(img_list))
        ...
        for img in img_list:
            print(img)
            self.H.Me(['cml','cp '+img+' '+str(M.root_dirname)+'/images/img_%d.jpg'%(i+1)])
            i+=1
        print(str(i)+' images copied!')
        self.H.Me(['cml','rm -r '+str(M.root_dirname)+'/images/flickr'])
