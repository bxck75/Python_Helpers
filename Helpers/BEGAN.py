...
class BEGAN:
    ...
    def build_model(self):
        ...
        self.k = tf.Variable(0, dtype=tf.float32, trainable=False)
        ...
        self.d_loss = real_reconstr_error - self.k*fake_reconstr_error
        self.g_loss = fake_reconstr_error
        
        self.M = real_reconstr_error + tf.abs(self.gamma*real_reconstr_error - fake_reconstr_error)
        self.k_update = self.k.assign(self.k+self.lambd*(self.gamma*real_reconstr_error-fake_reconstr_error))
    ...
    def train(self):        
        ...
        for index in range(self.epoch):
            ...
            for idx in range(batch_idxs):
                ...
                _, k_value, M_value = self.sess.run([self.k_update, self.k, self.M], feed_dict={self.input_data:batch_x, self.z:batch_z})

                iteration_time = time.time()
                total_time = (iteration_time - start_time)
                print("epoch[%d]:[%d/%d]: " %(index, idx, batch_idxs), "total_time:{:.4f}".format(total_time), "d_loss:{:.4f},g_loss:{:.4f},k_value:{:.4f}, M_value:{:.4f}".format(d_loss, g_loss,k_value, M_value))
